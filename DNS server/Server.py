...
class Server:
    ...
    def parse_field(self, response, i):
        ...
        if rtype in RECORDS:
            ...
            self.cache.add_rec(response[DNSRR][i].rrname.decode(),
                                  rtype,
    ...
    def is_in_cache(self, _packet):
        qname = _packet[DNSQR].qname.decode('cp1251')
        print(f'\nFor: {qname}\n')
        return qname in self.cache.data.keys() and \
               _packet[DNSQR].qtype in self.cache.data[qname] and \
               self.is_not_expired(qname, _packet)
